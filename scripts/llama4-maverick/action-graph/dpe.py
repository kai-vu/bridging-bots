...
def get_llm(api_key, llm_model):
    llm = Groq(model=llm_model, api_key=api_key, timeout=60)
    return llm

def define_schema():
    entities = ["Instruction", "Workflow", "Action", "Agent", "Component", 
                "Appliance", "Furniture", "Object", "Affordance", "Closing",
                "Opening", "Delivering", "Holding", "PickingUp", "PuttingDown",
                "Pulling", "Pushing", "Grasping"]
    relations = ["hasWorkflow", "hasAction", "precedes", "follows", 
                 "isPerformedBy", "actsOn", "isAffordedBy", "hasNaturalLanguage"]
...
if __name__ == "__main__":
    ...
    output_path = "../../../output/llama4-maverick/action-graph/dpe"
